...
export interface FilterOptions {
    ...
    /**
     * The SQL join type used for all joins generated by casl-bridge
     * (both ability-derived rule joins and external filter joins).
     *
     * - `"left"` (default) – `LEFT JOIN`; rows without a matching
     *   related entity are included with `NULL` values.
     * - `"inner"` – `INNER JOIN`; only rows with at least one
     *   matching related entity are returned.
     *
     * This setting affects only joins created internally by casl-bridge
     * and does **not** rewrite existing joins on a user-supplied query
     * builder.
     *
     * When `undefined` (the default) the existing behaviour (`LEFT JOIN`)
     * is preserved and existing SQL output is unchanged.
     */
    joinType?: 'left' | 'inner'
}
